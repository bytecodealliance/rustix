...
use rustix::net::{
    bind_unix, connect_unix, recvmsg, sendmsg_noaddr, sendmsg_unix, socket, socketpair,
    AddressFamily, Protocol, RecvAncillaryBuffer, RecvFlags, SendAncillaryBuffer, SendFlags,
    SocketAddrAny, SocketAddrUnix, SocketType,
};
...
fn server(ready: Arc<(Mutex<bool>, Condvar)>, path: &Path) {
    ...
    sendmsg_unix(
        &data_socket,
        match res.address {
            Some(SocketAddrAny::Unix(ref addr)) => addr,
