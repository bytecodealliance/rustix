...
use crate::imp::process::syscalls::{getgid, getpid, getuid};
...
/// Create a `[u8; N]` type buffer, where `N` is sized such that it fits the
/// provided types as control messages, as used by `sendmsg` and `recvmsg`.
///
/// # Examples
///
/// ```
...
// TODO: Should there exist a convenience wrapper that owns the buffer and
// potentially auto resizes?
...
impl<'a, T: FromCmsghdr<'a>> SocketAncillary<'a, T> {
    ...
    /// Returns a pointer to the next `cmsgshdr` data section to write to, if
    ...
    unsafe fn get_cmsg_data(
        ...
        while !cmsg.is_null() {
            ...
            // Most operating systems, but not Linux or emscripten, return the previous
            // pointer when its length is zero. Therefore, check if the
            // previous pointer is the same as the current one.
...
impl<'a> SocketAncillary<'a, SendAncillaryDataUnix<'a>> {
    ...
    /// Technically, that means this operation adds a control message with the
    /// level `SOL_SOCKET` and type `SCM_RIGHTS`.
    ...
    /// Technically, that means this operation adds a control message with the
    /// level `SOL_SOCKET` and type `SCM_CREDENTIALS` or `SCM_CREDS`.
    #[cfg(any(target_os = "android", target_os = "linux",))]
...
impl<'a, T> AncillaryDataIter<'a, T> {
    /// Create `AncillaryDataIter` struct to iterate through the data unit in
    /// the control message.
    ///
    /// # Safety
    ///
