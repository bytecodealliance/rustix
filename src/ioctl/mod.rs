...
///
/// `ioctl` was originally intended to act as a way of modifying the behavior of files, but has
/// since been adopted as a general purpose system call for making calls into the kernel. In
/// addition to the default calls exposed by generic file descriptors, many drivers expose their
/// own `ioctl` calls for controlling their behavior, some of which are proprietary.
///
/// This crate exposes many other `ioctl` interfaces with safe and idiomatic wrappers, like
/// [`ioctl_fionbio`](crate::io::ioctl_fionbio) and [`ioctl_fionread`](crate::io::ioctl_fionread).
/// It is recommended to use those instead of this function, as they are safer and more idiomatic.
...
///
/// Objects implementing this trait can be passed to [`ioctl`] to make an `ioctl` call. The contents
/// of the object represent the inputs to the `ioctl` call. The inputs must be convertible to a
/// pointer through the `as_ptr` method. In most cases, this involves either casting a number to
/// a pointer, or creating a pointer to the actual data. The latter case is necessary for `ioctl`
/// calls that modify userland data.
///
/// # Safety
///
/// This trait is unsafe to implement because it is impossible to guarantee that the `ioctl` call
/// is safe. The `ioctl` call may be proprietary, or it may be unsafe to call in certain
/// circumstances.
///
/// By implementing this trait, you guarantee that:
///
/// - The `ioctl` call expects the input provided by `as_ptr` and produces the output as indicated
///   by `output`.
/// - That `output_from_ptr` can safely take the pointer from `as_ptr` and cast it to the correct
///   type, *only* after the `ioctl` call.
...
/// - If `IS_MUTATING` is false, that no userland data will be modified by the `ioctl` call.
pub unsafe trait Ioctl {
    ...
    ///
    /// There are different types of opcode depending on the operation. See documentation for the
    /// [`Opcode`] struct for more information.
    const OPCODE: Opcode;

    /// Does the `ioctl` mutate any data in the userspace?
    ///
    /// If the `ioctl` call does not mutate any data in the userspace, then making this `false`
    /// enables optimizations that can make the call faster. When in doubt, set this to `true`.
    ///
    /// # SAFETY
    ///
    /// This should only be set to `false` if the `ioctl` call does not mutate any data in the
    /// userspace. Undefined behavior may occur if this is set to `false` when it should be `true`.
...
///
/// Note that this is relative to userland. `Read` means reading data from the kernel, and write
/// means the kernel writing data to userland.
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum Direction {
    /// None of the above.
    None,
