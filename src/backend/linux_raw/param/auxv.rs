...
unsafe fn init_from_auxp(mut auxp: *const Elf_auxv_t) -> Option<()> {
    ...
    loop {
        ...
        match a_type as _ {
            ...
            AT_PHDR => phdr = check_raw_pointer::<Elf_Phdr>(a_val)? as *mut Elf_Phdr,
            AT_PHNUM => phnum = a_val as usize,
            AT_PHENT => phent = a_val as usize,
            AT_EXECFN => execfn = check_raw_pointer::<c::c_char>(a_val)? as *mut c::c_char,
            AT_BASE => check_interpreter_base(a_val.cast())?,
            AT_SYSINFO_EHDR => sysinfo_ehdr = check_vdso_base(a_val.cast())? as *mut Elf_Ehdr,
            AT_NULL => break,
            _ => (),
        }
        auxp = auxp.add(1);
